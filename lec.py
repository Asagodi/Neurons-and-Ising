...
#        l_rate = np.min([error, 0.05])
        l_rate = np.exp(-step/(0.3*max_steps)-2.5)
...
def iteration(m_ia, h, J, max_steps, V_node, F_node, delta=10**-2):
    N = h.shape[0]
...
#                print(Mia, m_ia[node_i][ll])
                m_ia[node_i][ll]=Mia
                ll+=1
            if global_convergence(m_ia, m_ia_old, delta, N):
                return m_ia
...
def global_convergence(m_ia, m_ia_old, delta, N):
    s=0
    edges=0
    for i in range(N):
        for j in range(N-1):
...
def comput_mag_corre(m_ia, h, J, max_steps, V_node, F_node):
    N = h.shape[0]
    m_bp = zeros(N)
    c_bp = [[] for i in range(N)]
    for i in range(N):
        for j in range(N):
            c_bp[i].append(0.)
    for  it in range(N):
...
#                        print(m_ia[ib][b])
    ...
    for  it in range(int((N-1)*N/2)):
        ...
        for j in range(Kb):
            ...
            mia=m_ia[ib][b];
            product*=mia;
        corre=(tanh(J[it])+product)/(1.0+tanh(J[it])*product);
        i=neib[0];
        j=neib[1];
        if i<j:
            c_bp[i][j-1]=corre
...
def distance_entropy(m_ia, h, J, x, ref_sigma, V_node, F_node, beta=1.):
    h_p = beta*h + x*ref_sigma
    N = h.shape[0]
    J_p = [beta*J_ij for J_ij in J] 
#    J = #write func?
    a = 0
#    for i in range(N):
#        for j in range(i+1,N):
...
#                        print(m_ia[ib][b])
            ...
            for j in range(Kb-1):
                product01*=m_jb[j];
            product01*=tanh(J_p[itr]);
            suma+=log(cosh(J_p[itr])*(1.0+product01));				
            sumb+=log(cosh(J_p[itr])*(1.0-product01));
...
#                        print(m_ia[ib][b])
    ...
    for  it in range(int((N**2-N)/2)):
        ...
        for j in range(Ka):
            ...
            mia=m_ia[ib][b];
            product*=mia;
        Delta_Fa+=(Ka-1.0)*log(cosh(J_p[it])*(1.0+tanh(J_p[it])*product));
        E_a+=(Ka-1.0)*J_p[it]*(tanh(J_p[it])+product)/(1.0+tanh(J_p[it])*product);
        
    fe=(Delta_Fi-Delta_Fa)/float(N);
    eg=-(E_i-E_a)/float(N);
    entropy=fe+eg;
    return entropy, fe, eg
